...
class RoleModel extends CommonModel {
    ...
    function getGroupUserList($groupId) {
        ...
        $rs = $this->db->query('select b.id,b.nickname,b.email from '.$table.' as a ,'.$this->tablePrefix.'admin_user as b where a.level=1 and a.user_id=b.id and  a.role_id='.$groupId.' ');
        return $rs;
    }
    ...
    function setGroupUser($groupId,$userId) {
        $sql	=	"INSERT INTO ".$this->tablePrefix.'role_user (role_id,user_id,level) values ('.$groupId.','.$userId.',1)';
    ...
    function setGroupUsers($groupId,$userIdList) {
        ...
        $rs = $this->execute('INSERT INTO '.$this->tablePrefix.'role_user (role_id,user_id,level) SELECT a.id, b.id, 1 FROM '.$this->tablePrefix.$this->tableName.' a, '.$this->tablePrefix.'admin_user b WHERE '.$where);
        if($result===false) {
            return false;
        }else {
            return true;
